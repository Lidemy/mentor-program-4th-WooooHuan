function multiply(a, b) {
    ...
    for (let i = lA - 1; i >= 0; i--) {                                 // 字串 A 自個位數逐項調用
        for (let j = lB - 1; j >= 0; j--) {                             // 字串 B 自個位數逐項調用
            ...
            result[k] = Math.floor(mul / 10) + (result[i + j] || 0);    // 擷取結果的十位數，放入陣列供下一輪作為進位值調用，感謝同學 lauviah0622 協助偵錯
...
console.log(multiply('25', '25'));                                      // 142857 是一組很有趣的數字，詳見 : https://zh.wikipedia.org/wiki/142857
